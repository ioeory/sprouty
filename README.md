...
- **饼图分析**：按分类 / 项目 / 账本维度查看支出分布，支持年/月/全部时间；**仪表盘可按日历切换任意月份或年份**（上一月/下一月、月份选择、年份下拉，对应 API `year_month` / `year`）；查看**非当前自然月**时，剩余预算按所选月支出计算，「剩余天数 / 日均可花」仅在看**当月**时展示，避免历史视图误导；家庭账 + 多子账时与合并支出口径一致；分类图例随界面语言用中英名称实时切换（无需刷新）
- **流水与汇总**：流水列表 **分页**（加载更多）；顶部卡片上的 **支出/收入合计** 为当前筛选条件下 **全部记录** 之和（后端 `sum_expense` / `sum_income`），不仅是已加载条数；筛选区支持 **账单月份** 快捷填入该月起止日期
- **Telegram Bot**：发一条消息 `午餐50 l:报销` 即完成记账；支持 **`/install`**（别名 **`/fenqi`**）等额分期，例如 `/install 6 1800 餐饮 l:报销`（期数、总额、分类描述，规则与 Web 记一笔一致）
- **快速记账语法**：支持模糊输入、自然日期（昨天/前天/M月N号）、项目/账本跳转
- **外观与主题**：明暗（自动 / 明亮 / 暗黑）与 **站点配色**（默认靛蓝 / **自然绿**，新安装默认自然绿）分维设置，保存在浏览器 `localStorage`；顶栏调色板图标内可切换
...
- **按月查看**：仪表盘在「本月」模式下可切换到任意历史月份，「最近流水」在该模式下限定在所选月内（最新 5 条）；「本年 / 全部」下的最近流水仍为全局近期。流水页可用「账单月份」快速限定自然月，亦可手动改起止日期；分页时收支合计与列表筛选口径一致。
...
- **项目详情与流水**：在项目页进入详情后，可查看 / 编辑 / 删除归属该项目的记录（`GET /api/transactions?ledger_id=…&project_id=…`）；详情内「记一笔」默认选中当前项目。若当前路由为 **`/projects/:id`**，顶栏全局「记一笔」也会自动带上该项目及与详情页一致的统计账本

### 3. 记账

点右上角 `+` 或键盘 `N`：金额、分类、（可选）项目、（可选）多选标签，回车保存。支持 **等额分期**（多笔支出、同一分期组）。在项目详情页打开记一笔时，**项目**已默认选中当前项目。

### 4. Telegram Bot 绑定

1. 在 `设置 → Telegram` 点"生成绑定码"
2. 在 Telegram 找到你的 Bot，发送 `/start <绑定码>`
3. 收到"绑定成功"后，直接发消息就能记账；分期见 `/start` 帮助中的 `/install` 说明
...
│   ├── src/
│   │   ├── api/                axios client
│   │   ├── components/         AppLayout、ThemeProvider、记一笔弹窗等
│   │   ├── pages/              Dashboard / Transactions / Categories / Projects / ...
│   │   └── lib/                工具函数（如分类双语显示名）
│   └── nginx.conf              生产镜像用
